...
shinyServer(function(input, output) {
    ...
    getpca<-reactive({
        ...
        DOMINO_bp <- ggbiplot(DOMINO_pca, choices = c(input$Principal1,input$Principal2), ellipse = TRUE,  groups = DOMINO_countries, varname.size = 4)+ geom_point(aes(colour=DOMINO_countries), size = 3) + coord_cartesian(xlim = ranges2$x, ylim = ranges2$y) + ggtitle("PCA Bioplot of Data") + theme(axis.title = element_text(size = 16), plot.title = element_text(size = 20), legend.text = element_text(size = 14), legend.title = element_text(size = 16)) + scale_color_manual(values=cbbPalette) #Comparing PC1 and PC2
        ...
        #These are the different pca plots
            ...
            DOMINO_countries <- as_vector(DOMINO%>%
                                              dplyr::select(input$groups))
            DOMINO_bp <- ggbiplot(DOMINO_cat_pca, choices = c(input$Principal1,input$Principal2), ellipse = TRUE,  groups = DOMINO_countries, varname.size = 5)+geom_point(aes(colour=DOMINO_countries), size = 3)+coord_cartesian(xlim = ranges2$x, ylim = ranges2$y) + ggtitle("PCA Bioplot of Data") + theme(axis.title = element_text(size = 16), plot.title = element_text(size = 20), legend.text = element_text(size = 14), legend.title = element_text(size = 16)) + scale_color_manual(values=cbbPalette)
